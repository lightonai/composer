...
def apply_gradient_clipping(
    ...
    if fsdp_enabled:
        for module in model.modules():
            if isinstance(module, FullyShardedDataParallel) and module.check_is_root():
                if clipping_type == 'norm':
                    return module.clip_grad_norm_(max_norm=clipping_threshold).item()
                elif clipping_type == 'value':
                    return module.clip_grad_norm_(max_norm=clipping_threshold, norm_type=float('inf')).item()
                else:
        ...
        if clipping_type == 'adaptive':
            ...
            return torch.nn.utils.clip_grad_norm_(parameters, max_norm=clipping_threshold).item()
        elif clipping_type == 'value':
            return torch.nn.utils.clip_grad_value_(parameters, clip_value=clipping_threshold).item()
        else:
...
class GradientClipping(Algorithm):
    ...
    def apply(self, event: Event, state: State, logger: Logger) -> Optional[int]:
        ...
        if event == Event.AFTER_TRAIN_BATCH and not state.deepspeed_enabled:
            grad_norm = apply_gradient_clipping(
                model=state.model,
                clipping_type=self.clipping_type,
                clipping_threshold=self.clipping_threshold,
                fsdp_enabled=state.fsdp_enabled,
            )
            logger.log_metrics({"trainer/grad_norm": grad_norm})
...
#             if isinstance(module, FullyShardedDataParallel) and module.check_is_root():
#                 if clipping_type == 'norm':
#                     log.info(f"module.clip_grad_norm_(max_norm=clipping_threshold).item() : {module.clip_grad_norm_(max_norm=clipping_threshold).item()}")
#                     return module.clip_grad_norm_(max_norm=clipping_threshold).item()
#                 elif clipping_type == 'value':
#                     return module.clip_grad_norm_(max_norm=clipping_threshold, norm_type=float('inf')).item()
#                 else:
...
#         if clipping_type == 'adaptive':
#             _apply_agc(parameters, clipping_threshold=clipping_threshold)
#             return None # grad_norm not given by _apply_agc
#         elif clipping_type == 'norm':
#             return torch.nn.utils.clip_grad_norm_(parameters, max_norm=clipping_threshold).item()
#         elif clipping_type == 'value':
#             return torch.nn.utils.clip_grad_value_(parameters, clip_value=clipping_threshold).item()
...
#                 raise NotImplementedError(
#                     f"Deepspeed only supports gradient clipping of type 'norm' not of type '{self.clipping_type}'")
#         log.info(f"BEFORE : event : {event}, state.deepspeed_enabled : {state.deepspeed_enabled}")
#         if event == Event.AFTER_TRAIN_BATCH and not state.deepspeed_enabled:
#             grad_norm = apply_gradient_clipping(model=state.model,
#                                     clipping_type=self.clipping_type,
#                                     clipping_threshold=self.clipping_threshold,
#                                     fsdp_enabled=state.fsdp_enabled)
#             log.info(f"INSIDE : grad_norm : {grad_norm}")
#             logger.log_metrics({"trainer/grad_norm": grad_norm})
